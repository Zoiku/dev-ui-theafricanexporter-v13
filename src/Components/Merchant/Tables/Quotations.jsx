...
const Quotations = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 100,
      renderCell: ({ row }) => (
        <SmallPrimary onClick={() => selectQuote(row.id)}>View</SmallPrimary>
      ),
    },
  ];
              ...
              <div className="request-section-body">
                <div>
                  ...
                  <span className="toLower">
                    {selectedQuote.length} {selectedQuote.lengthUnit}
                  </span>
                </div>
                {selectedQuote.product === PRODUCTS.TEAK_SQUARE_LOGS && (
                  <>
                    <div>
                      <span>Thickness:</span>
                      <span className="toLower">
                        {selectedQuote.thickness} {selectedQuote.thicknessUnit}
                      </span>
                    </div>
                    <div>
                      <span>Width:</span>
                      <span className="toLower">
                        {selectedQuote.width} {selectedQuote.widthUnit}
                      </span>
                    </div>
                    <div>
                      <span>Drying:</span>
                ...
                {selectedQuote.product === PRODUCTS.TEAK_ROUND_LOGS && (
                  <>
                    <div>
                      <span>Diameter:</span>
                      <span className="toLower">
                        {selectedQuote.diameter} {selectedQuote.diameterUnit}
                      </span>
                    </div>
                  </>
                )}
                <div>
                  <span>Quantity:</span>
                  <span>
                    {selectedQuote.quantity} {selectedQuote.containerSize}
                  </span>
                </div>
              </div>
            </section>
                  ...
                  <table>
                    <thead>
                      <tr className="incoterm-setting-heading-container">
                        <th>Number</th>
                        {selectedQuote?.product === "Teak Round Logs" &&
                          selectedQuote?.terms !== "FOB" && (
                            <th>Diameter (cm)</th>
                          )}
                        <th>CBM</th>
                        <th>Price per CBM</th>
                        ...
                        {(selectedQuote.terms === "CIF" ||
                          selectedQuote.terms === "CFR") && (
                          <th>Total Freight</th>
                        )}
                        <th>Total amount</th>
                      </tr>
                      ...
                      {selectedQuote.incotermRows.length > 0 ? (
                        ...
                        <tr>
                          ...
                          {(selectedQuote.terms === "CIF" ||
                            selectedQuote.terms === "CFR") && (
                            <td>
                              <input disabled name="diameter" value="--" />
                            </td>
                          )}
                          <td>
                            <input disabled name="cbm" value="--" />
                          </td>
                          <td>
                            <input disabled name="noOfPieces" value="--" />
                          </td>
                          <td>
                            <input disabled name="price" value="--" />
                          </td>
                          <td>
                            <input disabled name="totalPrice" value="--" />
                          </td>
                          {selectedQuote.terms === "CIF" && (
                            <td>
                              <input disabled name="insurance" value="--" />
                            </td>
                          )}
                          {(selectedQuote.terms === "CIF" ||
                            selectedQuote.terms === "CFR") && (
                            <td>
                              <input disabled name="freight" value="--" />
                            </td>
                          )}
                          <td>
                            <input disabled value="--" />
      ...
      try {
        ...
        if (errors.length === 0) {
          ...
          filteredData.map((quote, index) => {
            ...
            quote.expiryDate = new Date(quote.request.expiryDate)
              .toUTCString()
              .slice(0, 16);
            ...
            quote.diameter =
              quote.request.quotationProducts[0].specification.diameter;
            quote.diameterUnit =
              quote.request.quotationProducts[0].specification.diameterUnit;
            quote.drying =
              quote?.request?.quotationProducts[0]?.specification?.drying?.label;
            quote.length =
              quote?.request?.quotationProducts[0]?.specification.length;
            quote.thickness =
              quote?.request?.quotationProducts[0]?.specification?.thickness;
            quote.width =
              quote?.request?.quotationProducts[0]?.specification?.width;
            quote.lengthUnit =
              quote?.request?.quotationProducts[0]?.specification.lengthUnit;
            quote.thicknessUnit =
              quote?.request?.quotationProducts[0]?.specification.thicknessUnit;
            quote.widthUnit =
              quote?.request.quotationProducts[0]?.specification?.widthUnit;
            quote.species =
              quote.request.quotationProducts[0].product.species.label;
            quote.speciesType =
              quote.request.quotationProducts[0].product.species.type.label;
            quote.origin =
              quote.request.quotationProducts[0].product.origin.country;
            quote.containerSize =
              quote.request.quotationProducts[0].product.supportedShippingContainers[0].label;
            quote.volume =
              quote.request.quotationProducts[0].product.volume.value;
            quote.volumeUnit =
              quote.request.quotationProducts[0].product.volume.unit;
    ...
    <div className="Quotes-Table">
      ...
      <DataGrid
        components={{
          ...
          NoRowsOverlay: () => <Overlay label="Quotations" />,
        }}
