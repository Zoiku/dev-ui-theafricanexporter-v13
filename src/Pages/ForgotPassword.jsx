...
const ForgotPassword = () => {
  ...
  const handleSendPasswordLink = async (e) => {
    ...
    try {
      const { errors } = await authService.sendResetPasswordLink(
        state.payload.email
      );
      if (errors.length === 0) {
        ...
        handleFailedRequest(
          "The email submited is not a registered account",
          7000
        );
      }
    } catch (error) {}
  ...
  const handleSubmitPassword = async (e) => {
    ...
    try {
      ...
      if (errors.length === 0) {
        ...
        dispatch({ type: REQUEST_FAILED });
        handleFailedRequest(
          "Problem resetting your password, please try again",
          7000
        );
      }
    } catch (error) {}
        ...
        <div className="left-container">
          <div className="go-back-container">
            <NavLink className="go-back" style={navLinkStyle} to="/login">
              <KeyboardBackspaceRoundedIcon className="go-back-items" />
              <span className="go-back-items">Back to login</span>
            </NavLink>
          </div>
